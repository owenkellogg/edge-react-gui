...
import { createNewWalletsAndTokens, MainWalletCreateItem, PLACEHOLDER_WALLET_ID, splitCreateWalletItems } from '../../actions/CreateWalletActions'
  ...
  const handleNext = useHandler(async () => {
    ...
    for (const pluginId of uniquePluginIdList) {
      ...
      if (existingWalletIds.length === 0) {
        ...
        newTokenItems.forEach(item => {
          if (item.pluginId === pluginId) {
            item.createWalletIds = pluginIdWalletIdsMap[pluginId]
          }
        })
      } else {
    ...
    if (newWalletItems.length > 0) {
      ...
      await dispatch(createNewWalletsAndTokens(newTokenItems, {}, ''))
